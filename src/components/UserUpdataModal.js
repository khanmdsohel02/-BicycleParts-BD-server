...
const UserUpdataModal = () => {
    ...
    const handleUudateUser = async event => {
      ...
      const client =  event.target.user.value
      const phone =  event.target.phone.value
      const url =  event.target.url.value
      await updateProfile({ displayName: client, phoneNumber:phone, photoURL:url});
  }
...
<div class="modal modal-bottom sm:modal-middle">
  ...
  <form onSubmit={handleUudateUser} className='grid grid-cols-1 gap-4 justify-items-center mt-4'>
    ...
    <input type="text" name='user'  placeholder={user?.displayName}  className="input input-bordered w-full max-w-xs" />
